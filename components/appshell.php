...
foreach($items as [$p,$label,$icon]){ $allowed=match($p){'users','audit','settings'=>in_array($u['role_name'],['SYSTEM ADMIN'],true),'templates'=>in_array($u['role_name'],['SYSTEM ADMIN','HR / CLINIC ADMIN'],true),'approvals'=>$u['role_name']==='SUPERVISOR',default=>true}; if($allowed) echo '<a class="'.($active===$p?'active':'').'" href="index.php?page='.$p.'"><span>'.$icon.'</span>'.e($label).'</a>'; }
